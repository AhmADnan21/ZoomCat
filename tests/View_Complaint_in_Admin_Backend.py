...
class TestComplaintsView(unittest.TestCase):
    def setUp(self):
        """
        ...
        self.report_file = os.path.join(self.reports_dir, f"test_view_complaints_{current_time}.txt")
        ...
        try:
            ...
            if os.path.exists(self.reports_dir):
                report_files = sorted([f for f in os.listdir(self.reports_dir) 
                                    if f.startswith('test_view_complaints_') and f.endswith('.txt')])
